...
// ฟังก์ชันดึงข้อมูลจาก Firebase และสร้างตาราง
function loadData() {
  ...
  db.ref("assessments").once("value", snapshot => {
    ...
    if(!data) return tableContainer.innerHTML = "<p>ยังไม่มีข้อมูล</p>";

    // สร้างตาราง HTML
    let html = "<table border='1'><tr><th>UpperPosture</th><th>UpperTime</th><th>LowerPosture</th><th>LowerTime</th><th>Force</th><th>Repetition</th><th>Twist</th><th>Timestamp</th></tr>";
    const chartData = {labels: [], totals: []};

    Object.values(data).forEach(item => {
      html += `<tr>
        <td>${item.upperPosture}</td>
        <td>${item.upperTime}</td>
        <td>${item.lowerPosture}</td>
        <td>${item.lowerTime}</td>
        <td>${item.force}</td>
      ...
      // สำหรับกราฟ: คะแนนรวมง่าย ๆ
      const total = (1 + parseInt(item.upperTime || 0)) * (1 + parseInt(item.lowerTime || 0)) 
                    + parseInt(item.force || 0) + parseInt(item.repetition || 0) + parseInt(item.twist || 0);
      chartData.labels.push(new Date(item.timestamp).toLocaleDateString());
